...
\begin{proof}
    ``$\Rightarrow$'' По определению $f' \quad \frac{f(x+h)-f(h)}{h}\geq 0$
...
\begin{consequence}
    ...
    $f=\const\Leftrightarrow (f\in C(\langle a,b\rangle)$ --- дифф. на $\langle a,b\rangle, f'\equiv0)$
\end{consequence}
\begin{consequence}
...
Аналогично определеяется минимум.
...
\begin{theorem}
    Необходимость и достаточность условия локального экстремума
...
\begin{theorem}
    ...
    $f\in C(\langle a,b\rangle)$ тогда у $f$ существует первообразная.
\end{theorem}
%</осуществованиипервообразной>
\begin{proof}
    Чуть позже.
...
$$\intt x^ndx=\frac{x^{(n+1)}}{n+1}, n\not=-1$$
$$\intt \frac{1}{x}dx=\ln x$$
$$\intt \sin x dx = -\cos x$$
$$\intt \cos x dx = \sin x$$
$$\intt e^xdx = e^x$$
$$\intt \frac{1}{\sqrt{1-x^2}}=\arcsin x$$
$$\intt \frac{1}{\sqrt{1+x^2}} dx=\ln(x+\sqrt{1+x^2}) \text{ --- длинный логарифм}$$
$$\intt \frac{1}{\cos^2 x}=\tg x$$
$$\intt \frac{1}{\sin^2 x}=-\ctg x$$
\textcolor{red}{Почему где-то нет $dx$? Кохась забыл?}
%</таблицапервообразных>
...
\begin{theorem}
    ...
    \begin{enumerate}
        ...
        % TODO: fix the подставление operator 
...
$$=\intt \frac{\cos t dt}{\cos^2 t}=\intt \frac{\cos t dt}{1-\sin^2 t}=[y:=\sin t]=\intt \frac{dy}{1-y^2}=\intt \frac{1}{1-y}\cdot\frac{1}{1+y}=\intt \frac{1}{2}\left(\frac{1}{1-y}+\frac{1}{1+y}\right)dy=\frac{1}{2}\left(-\ln(1-y)+\ln(1+y)\right)=$$
$$=\frac{1}{2}\ln\frac{1+y}{1-y}=\frac{1}{2}\ln\frac{1+\sin t}{1-\sin t}=\frac{1}{2}\ln\frac{1+\sin \arctg x}{1-\sin\arctg x}$$
...
\begin{proof}
    ...
    $$\exists \varepsilon>0 \ \ \forall \delta > 0 \ \ \exists x_\delta, \overline x_\delta : \rho(x_\delta, \overline x_\delta) < \delta \quad \rho(f(x_\delta), f(\overline x_\delta))\geq\varepsilon$$
    ...
    $\rho(\tilde x, \dbltilde x)\leq 0$, т.е. $\tilde x = \dbltilde x$. Тогда $f(x_{n_k})\to f(\tilde x), f(\overline x_{n_k})\to f(\tilde x)$, противоречие с $\rho(f(x_n), f(\overline x_n))\geq\varepsilon$
\end{proof}
...
\begin{proof}
    ...
    Возьмём $n: \frac{1}{n}<\delta$

    Построим доску $Hex(n+1, n+1)$, где $n+1$ --- число узлов.

    Логические координаты узла $(v_1, v_2)\quad v_1,v_2\in\{0\ldots n\}$ имеют физические координаты $\left(\frac{v_1}{n}, \frac{v_2}{n}\right)$

    $K(V):=\min\{i\in\{1,2\} : |f(\frac{v}{n}) - \frac{v_i}{n}|\}$
