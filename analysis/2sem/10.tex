...
\begin{example}
    ...
    Правая часть --- среднее значение $f$ на $[a,b]$.

    Левая часть --- среднее геометрическое.
    $$\exp\left(\frac{1}{b-a} \int_a^b \ln f(x)dx \right) = \exp\left(\sum \frac{1}{n} \ln f(x_i) \right)=\prod_{i=1}^n \exp\left(\frac{\ln(f(x_i))}{n}\right)=\sqrt[n]{f(x_i)\cdots f(x_n)}$$
