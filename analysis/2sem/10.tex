...
\begin{proof}
    ...
    Покажем, что $x^*:=\alpha_1x_1+\ldots+\alpha_nx_n \in \langle a, b\rangle$:
    $$\min x_i\leq x^* \leq(\alpha_1+\ldots+\alpha_n)\max(x_i)=\max(x_i) \Rightarrow x^*\in \langle a,b \rangle$$
...
\begin{example}
	%<*интегральноенеравенствокоши>
	Неравенство Коши для интегралов:
    $$f>0, f\in C[a,b] \quad \exp\left(\frac{1}{b-a} \int_a^b \ln f(x)dx \right) \leq \frac{1}{b-a}\int_a^b f(x)dx$$
    Правая часть --- среднее значение $f$ на $[a,b]$, похоже на среднее арифметическое, если рассмотреть интегральную сумму:
    $$\frac{1}{b-a}\int_a^b f \approx \frac{1}{b-a} \sum \frac{1}{n}f(x_i)$$

    Аналогично левая часть $\approx$ среднее геометрическое:
    $$\exp\left(\frac{1}{b-a} \int_a^b \ln f(x)dx \right) \approx \exp\left(\sum \frac{1}{n} \ln f(x_i) \right)=\prod_{i=1}^n \exp\left(\frac{\ln(f(x_i))}{n}\right)=\sqrt[n]{f(x_i)\cdots f(x_n)}$$
    ...
    \begin{itemize}
        ...
        \item $\varphi\leftrightarrow f$
    \end{itemize}
    % cлева из йенсена, справа из текущего
    %</интегральноенеравенствокоши>
\end{example}
...
\begin{proof}
    $f(x)=x^p, (p>1)$ --- строго выпуклая, т.к. $f''=p(p-1)x^{p-2}>0$
    
    По Йенсену $\left(\sum \alpha_i x_i\right)^p\leq \sum \alpha_i x_i^p$

    $$\alpha_i := \frac{b_i^q}{\sum b_j^q} \quad x_i := a_ib_i^{\frac{-1}{p-1}}\left(\sum b_j^q\right)$$

    $$\text{Левая часть}^{\frac{1}{p}}=\sum_{i=1}^n\left(\frac{b_i^q}{\sum b_j^q} a_ib_i^{\frac{-1}{p-1}} \sum b_j^q\right)=\sum a_ib_i^{q-\frac{1}{p-1}}=\sum a_ib_i$$
    $$\text{Правая часть}=\sum\frac{b_i^q}{\sum b_j^q} a_i^pb_i^{\frac{-p}{p-1}}\left(\sum b_j^q\right)^p=\sum\frac{b_i^q}{\sum b_j^q} a_i^pb_i^{-q}\left(\sum b_j^q\right)^p=$$
    $$=\left(\sum a_i^p\right)\left(\sum b_j^q\right)^{p-1}=\left(\sum a_i^p\right)\left(\sum b_j^q\right)^{\frac{p}{q}}$$
    $$\text{Правая часть}^{\frac{1}{p}}=\left(\sum a_i^p\right)^{\frac{1}{p}}\left(\sum b_j^q\right)^\frac{1}{q}$$
\end{proof}
%</неравенствогельдераproof>

%<*неравенствогельдераобщийвид>
...
\begin{theorem}
    ...
    \begin{enumerate}
        ...
        \item $K_n\geq 1$ для бесконечного множества $n$ $\Rightarrow$ $\sum a_n$ расходится.
    \end{enumerate}
...
\begin{proof}
    Lite:
    \begin{enumerate}
        \item НСНМ $\sqrt[n]{a_n} \leq q \Leftrightarrow a_n\leq q^n$, $\sum q^n$ сх. $\Rightarrow \sum a_n$ сх.
...
\begin{theorem}
    ...
    \begin{enumerate}
        ...
        \item $R_n\leq 1$ НСНМ $\Rightarrow \sum a_n$ расх.
    \end{enumerate}
    %</признакраабе>
\end{theorem}
