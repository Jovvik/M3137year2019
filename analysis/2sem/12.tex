...
\begin{theorem}
    ...
    \begin{enumerate}
        ...
        \item $b_k$ монотонна, ограничена: $\exists C_B > 0 \ \ \forall k \ \ |b_k|<C_B$
    \end{enumerate}
...
\begin{definition}
    ...
    \textbf{Предел вдоль пути} \textit{(непрерывного)} $\gamma : [-\alpha, \alpha]\to\R^m$ функции $f : \R^m \to \R$:
    $$\lim_{t\to0} f(\gamma(t))$$
    %</пределвдольпути>
\end{definition}
...
$$f:\R^m \to\R^n \text{ --- лин.} \quad \forall \alpha,\beta\in\R \ \ \forall x,y\in\R^m \ \ f(\alpha x + \beta y)=\alpha f(x) + \beta f(y)$$
%</линейныйоператор>
...
$F:\mathbb{C}\to\mathbb{C} \quad a\in\mathbb{C}$
\begin{definition}
    $F$ \textbf{комплексно дифференциируема} в точке $A$, если $\exists \lambda\in\mathbb{C}$:
