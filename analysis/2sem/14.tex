...
$\sum -\frac{x^2}{2k^2}+o\left(\frac{1}{k^2}\right)$ очевидно сходится, кроме того, $x$ может быть $\le 0$, сходимость не изменится. $\Gamma(x)$ не определено для $x<0$, но если рассматривать формулу Вейерштрасса как определение $\Gamma$, то для $x\in\N^-$ $\Gamma(x)=+\infty$, т.к. $\prod 1+a_k=0$. Кроме того, можно подставлять и $x\in \mathbb{C}$.
