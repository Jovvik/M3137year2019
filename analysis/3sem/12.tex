...
\begin{theorem}
    \[\forall z, w\in \mathbb{C} \ \ \exp(z + w) = \exp z \cdot \exp w\]
\end{theorem}
\begin{proof}
...
\begin{remark}
    \begin{enumerate}
        ...
        \item [3'] \(\forall A, B\in \mathfrak{A}, A\cap B =\emptyset \ \ \mu(A\cup B) = \mu A + \mu B\)
    \end{enumerate}
\end{remark}
...
\begin{proof}\itemfix
    \begin{enumerate}
        \item Усиление аксиомы 3 из определения полукольца:
              ...
              Это было доказано ранее, т.к. по теореме \(A = \left( \bigsqcup A_i \right) \cup \left( \bigsqcup B_l \right)\) --- дизъюнктное объединение конечного числа множеств.
