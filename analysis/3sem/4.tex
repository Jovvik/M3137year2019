...
\begin{theorem}[о неявном отображении]\itemfix
    ...
    \[\frac{\partial F}{\partial y} = \begin{pmatrix}
        ...
        \end{pmatrix}\]
    Пусть \((a, b) = (a_1 \dots a_m, b_1 \dots b_n)\) --- решение системы и \(\det \left(\frac{\partial F}{\partial y} (a, b)\right) \neq 0\). Тогда \(\exists U(a) \subset \R^m\) и \(\exists ! \ \Phi\) такие, что \(\forall x\in U(a)\) \(x, \Phi(x)\) --- тоже решение системы.
    %</онеявномотображениивтерминахсистемы>
\end{theorem}
    ...
    \begin{itemize}
        ...
        \item [$1$:] $\tilde F: O\to\R^{m+n} : (x, y)\mapsto(x, F(x, y)), \tilde F(a, b) = (a, 0)$
              ...
              \begin{tikzpicture}
                  ...
                  \draw[green, thick] (0, 3) -- (0, 2) node[anchor=south east] {$Q$};
                  ...
                  \draw[green, thick] (11, 0) -- (10, 0) node[anchor=north east] {$P$};
                  \filldraw[black] (10.5, 0) circle (1pt) node[anchor=south] {$a$};
                  \draw[] (10.5, 0) \irregularcircle{0.5cm}{1mm} node[anchor=south west] {$V$};
              ...
              \begin{enumerate}
                  ...
                  \item ``Ось $x$'' $\Leftrightarrow$ ``ось $u$'', $P:=``\text{ось } u\text{''} = \R^m\times{a}\cap V$, $P$ --- откр. в $\R^m$, $P=P_1$
                  \item $\Phi(x) := H(x, 0)$
...
\begin{definition}\itemfix
    \begin{itemize}
        ...
        \item $k\in\{1 \dots m\}$
    \end{itemize}
    $M$ --- \textbf{простое $k$-мерное \textit{(непрерывное)} многообразие} в $\R^m$, если оно гомеоморфно некоторому открытому множеству $O\subset\R^k$
...
\begin{example}\itemfix
    \begin{enumerate}
        \item Полусфера в $\R^3 = \{(x, y, z)\in\R^3 : z > 0, x^2 + y^2 + z^2 = r^2\}$
...
\begin{proof}\itemfix
    \begin{itemize}
        \item [$1 \Rightarrow 2$:] $\Phi$ --- параметризация $O\subset\R^k\to\R^m, \Phi\in C^r, p=\Phi(t^0)$
              ...
              Множество $\Phi(W)$ открыто в $M$ по топологическому определению непрерывности $\Rightarrow \Phi(W) = M \cap \tilde U$, где $\tilde U$ открыто в $\R^m$

              Можно считать, что $\tilde U\subset V\times \R^{m-k}$, т.к. \(\R^{m - k}\) открыто.
              ...
              $\begin{pmatrix}
                  ...
                  \end{pmatrix} = \begin{pmatrix}
                      ...
                      \vdots                                & \ddots & \vdots                                & 0  & -1 & \ddots & \vdots \\
        ...
        \item [$2 \Rightarrow 1$:] $F := (f_1\ldots f_{m-k})$
              ...
              $F(x_1\ldots x_k, x_{k+1}\ldots x_m) = 0$ при $x\in \tilde U$
              ...
              $\exists H\in C^r : P\to Q : F(x', H(x')) \equiv 0$ для $x'\in P$

              Тогда $\Phi : P\to\R^m : (x_1\ldots x_k)\mapsto(x_1\ldots x_k, H_1(x_1\ldots x_k), H_2(x_1\ldots x_k) \ldots H_{m-k}(x_1\ldots x_k))$

              $\Phi$ --- гомеоморфизм $P$ и $M \cap \tilde U$, потому что $\Phi^{-1}$ --- фактически проекция.
...
\begin{proof}\itemfix
    \begin{itemize}[align=left]
        \item [\textbf{Частный случай}:] Пусть $\rg \Phi'_1(t^0), \rg \Phi'_2(s^0)$ достигается на первых $k$ столбцах.

              Тогда $\Phi_1 = \Phi_2 \circ \underbrace{(L \circ \Phi_2)^{-1} \circ (L \circ \Phi_1)}_{\Theta \text{ --- искомый диффеоморфизм}}$

              \begin{figure}[h]
                  \centering
                  \includesvg[scale=0.8]{images/о_двух_параметризациях.svg}
              \end{figure}

        \item [\textbf{Общий случай}:] $\Phi_1 = \Phi_2 \circ (\Phi_2 \circ L_2)^{-1} \circ (L_2 \circ L_1^{-1}) \circ (L_1\circ \Phi_1)$

              Кажется, в формуле ошибка --- см. иллюстрацию.

              %   \begin{figure}[h]
              %   \centering
              \includesvg[scale=0.8]{images/о_двух_параметризациях_2.svg}
              %   \end{figure}

              $$L_2 \circ L_1^{-1} = L_2 \circ \Phi_1 \circ (L_1 \circ \Phi_1)^{-1} \in C^r$$
