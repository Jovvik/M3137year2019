...
\begin{proof}
    ...
    \end{equation}
    $\Rightarrow \forall x_0\in X$ вещественная последовательность $(f_n(x_0))$ фундаментальная $\Rightarrow \exists\lim\limits_{n\to+\infty} f_n(x_0) =: f(x_0)$, тогда $f$ --- поточечный предел $f_n$. Проверим, что \(f_n \rightrightarrows f\) и \(f\in C(X)\).

    В \eqref{фундпоследовательность} перейдем к пределу при $m\to+\infty$:
    $$\forall \varepsilon > 0 \ \ \exists N \ \ \forall n \ \ \forall x\in X \ \ |f_n(x) - f(x)| \le \varepsilon \Rightarrow f_n\xrightrightarrows[X]{}f \xRightarrow{\text{Стокс}} f\in C(X)$$
\end{proof}
%</метрикавcxproof>
