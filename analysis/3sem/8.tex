...
\begin{proof}\itemfix
    \begin{itemize}
        ...
        \item [2$\Rightarrow$3] $\gamma$ --- петля: $[a, b]\to O$. $\gamma(a) = \gamma(b) = A$
              ...
              Рассмотрим постоянный путь $\tilde \gamma : [a, b]\to O, t\mapsto A$. По свойству 2: $\int_\gamma = \int_{\tilde \gamma} \langle V, \gamma' \rangle dt = 0$
        \item [3$\Rightarrow$2] $\gamma_1, \gamma_2$ --- пути с общим началом и концом. Тогда $\gamma := \gamma_2^- \gamma_1$ --- петля. $\gamma$ --- кусочно гладкий $\Rightarrow 0 = \int_\gamma = \int_{\gamma_1} + \int_{\gamma_2^-} = \int_{\gamma_1} - \int_{\gamma_2}$
...
\begin{proof}
    ...
    \begin{align}
        \frac{\partial f}{\partial x_j} (x) & = \text{правило Лейбница}                                                                                                        \\
                                            ...
                                            & = V_j(x)
    \end{align}
    \eqref{по производной поля}: по \eqref{производные поля}.
\end{proof}
%</леммапуанкареproof>
...
\begin{remark}
    %<*дифференцируемостьгаммафункции2>
    \begin{align*}
    ...
    \begin{enumerate}
        ...
        \item $\sum a_n \stackrel{?}{=} \lim\limits_{x\to x_0} \sum u_n(x)$
              ...
              $\tilde u_n(x) = \begin{cases}
                      ...
                      a_n,    & x=x_0
                  \end{cases}$ --- задано на $E\cup \{x_0\}$, непрерывно в $x_0$.
              ...
              $\sum \tilde u_n(x)$ --- равномерно сходится, т.к.:
...
\begin{manualtheorem}{4}[о перестановке двух предельных переходов]\itemfix
    ...
    \end{enumerate}
    То есть пунктирные преобразования верны:
    $$\begin{tikzcd}[ampersand replacement=\&]
            f_n \arrow[r, yshift=0.7ex]{u}{n\to+\infty}\arrow[r, yshift=-0.7ex] \arrow[swap]{d}{x\to x_0} \& S(x) \arrow[dashed]{d}{x\to x_0} \\
