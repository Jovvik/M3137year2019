...
\begin{proof}
    ...
    \begin{figure}[h]
        \centering
        \includesvg[scale=0.7]{\detokenize{images/приближение_ступенчатой.svg}}
    \end{figure}
...
\begin{proof}
    ...
    \(e(f < a) \subset e\), \(\lambda_m\) --- полная в \(\R^m\)\footnote{Любое подмножество множества нулевой меры измеримо.} \( \Rightarrow e(f < a)\) --- измеримо в \(E\).
...
\begin{definition}
    ...
    Если \(f_n(x) \xrightarrow[n \to +\infty]{} f(x)\) при п.в. \(x \in E\), тогда говорят, что \(f_n\) \textbf{сходится на \(E\) почти везде}.
    %</сходимостьпочтивезде>
\end{definition}
...
\begin{prop}\itemfix
    \begin{enumerate}
        ...
        \item В условии пункта один можно переопределить \(f\) на множестве \(e\). Тогда получится \(\tilde{f}\) и \(f_n(x) \to \tilde{f}(x)\) почти везде и \(\tilde{f}\) измеримо.
...
\end{exercise}
\begin{proof}
    Пусть существует множество \(X(|f - g| > \varepsilon)\) положительной меры.
    \[X(|f - g| > \varepsilon) \subset X(|f - f_n| > \varepsilon) \cup X(|g - f_n| > \varepsilon)\]
    \[\mu X(|f - g| > \varepsilon) \leq \underbrace{\mu X(|f - f_n| > \varepsilon) + \mu X(|g - f_n| > \varepsilon)}_{ \to 0}\]
    Следовательно предположение неверно. Но тогда \(\mu(\bigcup X(|f - g| > \varepsilon)) = 0\), а следовательно \(f \sim g\).

    Аналогичное доказательство есть в более подробном виде по следующей \href{https://math.stackexchange.com/questions/1994185/proving-for-a-lebesgue-measure-the-convergence-in-measure-is-equivalent-to-the-c}{ссылке}
\end{proof}
...
\begin{example}\itemfix
    \begin{enumerate}
        \item \(f_n(x) = \frac{1}{nx}, x > 0, X = \R_{ +}, f \equiv 0\)
              ...
              \[X(|f_n - f| \geq \varepsilon) = X\left(\frac{1}{nx} \geq \varepsilon\right) = X\left(x \leq \frac{1}{\varepsilon n}\right)\]
        ...
        \item \(n = 2^k + l, 0 \leq l < 2^k, X = [0, 1], \lambda\)
              ...
              \(\lim f_n(x)\) не существует ни при каком \(x\)!\footnote{Это восклицательный знак, а не факториал.}
