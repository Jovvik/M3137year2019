    ...
    \begin{itemize}
        ...
        \item [ \( \geq \)]
              \begin{enumerate}
                  ...
                  \item Для \(n \in \N : A = \bigsqcup_{i = 1}^n A_i\) тривиально по индукции.
                  \item \(A = \bigsqcup_{i = 1}^n A_i \cup B_n\), где \(B_n = \bigsqcup_{i > n} A_i\)

                        \(\int_{B_n} f \geq 0\), т.к. \(f \geq 0\). Таким образом:
                        \[\int_A f = \sum_{i = 1}^n \int_{A_i} f + \int_{B_n} f \geq \sum_{i = 1}^n \int_{A_i} f\]
              \end{enumerate}
    \end{itemize}
\end{proof}
%</счётнаяадиитивностьинтегралаproof>
...
%<*счётнаяадиитивностьинтегралаcorollary>
...
\begin{proof}\itemfix
    \begin{enumerate}
        ...
        \item \(f \geq 0\), измеримо. \(\exists \text{ступ. } f_n : 0 \leq f_n \leq f_{n+1} \leq \dots \ \ \lim f_n = f\)
              ...
              \[f_n + g_n \to f + g\]
              \[\int_E f + \int_E g \xleftarrow{\text{т. Леви}} \int_E f_n + \int_E g_n \xlongequal{\text{пункт 1}} \int_E f_n + g_n \xrightarrow{\text{т. Леви}} \int_E f + g\]
    \end{enumerate}
\end{proof}
%</линейностьинтегралаproof>

%<*линейностьинтегралаcorollary>
\begin{corollary}
    \(f, g\) суммируемы на \(E\). Тогда \(f + g\) суммируемо и \(\int_{E} f + g = \int_E f + \int_E g\). Таким образом, доказано~\ref{доказано позже}.
\end{corollary}
%</линейностьинтегралаcorollary>

%<*линейностьинтегралаcorollaryproof>
...
\begin{theorem}[об интегрировании положительных рядов]\itemfix
    %<*обинтегрированииположительныхрядов>
    \begin{itemize}
        \item \((X, \mathfrak{A}, \mu)\) --- пространство с мерой
    ...
    %</обинтегрированииположительныхрядов>
\end{theorem}
%<*обинтегрированииположительныхрядовproof>
\begin{proof}
    По теореме Леви:

    \[S_n : = \sum_{k = 1}^n u_k \quad 0 \leq S_n \leq S_{n+1} \leq \dots \]
    \(S_n \to S = \sum\limits_{k = 1}^{+\infty}u_k\), тогда \(\int_E S_n \to \int_E S\)
    \[\int_E \left( \sum_{n = 1}^{+\infty} u_n(x) \right) = \int_E S \leftarrow \int_E S_n \xlongequal{\text{линейность } \int} \sum_{k = 1}^n \int_E u_k\]
\end{proof}
%</обинтегрированииположительныхрядовproof>

%<*обинтегрированииположительныхрядовcorollary>
\begin{corollary}
    \(u_n\) измеримо и
    \(\sum\limits_{n = 1}^{+\infty} \int_E |u_n| < +\infty\). Тогда ряд \(\sum u_n(x)\) абсолютно сходится при почти всех \(x\).
\end{corollary}
%</обинтегрированииположительныхрядовcorollary>
%<*обинтегрированииположительныхрядовcorollaryproof>
\begin{proof}
    \[S(x) : = \sum |u_n(x)|\]
...
\begin{proof}
    ...
    \begin{align*}
        \int_{[ - N, N]} \frac{|a_n| d\lambda}{\sqrt{|x - x_n|}}
         ...
         & \symref{по чётности}{\leq} |a_n|\int_{ - N}^{N} \frac{dx}{\sqrt{|x|}} \\
         & = 4 \sqrt{N} |a_n|                                                    \\
        \sum_n\int_{[ - N, N]} \frac{|a_n| d\lambda}{\sqrt{|x - x_n|}}
         & \leq 4 \sqrt{N} \sum|a_n| < +\infty
    \end{align*}
    \blfootnote{\eqref{по чётности}: Т.к. \(\frac{1}{\sqrt{|x|}}\) чётна и при этом для \(x \geq 0\) убывает, площадь под симметричным относительно \(0\) отрезком максимальна среди всех отрезков такой же длины.}
\end{proof}
