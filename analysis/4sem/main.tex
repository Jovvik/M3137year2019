...
\begin{corollary}[следствия]
    \(\mathcal{L}(X)\) --- линейное пространство, а отображение \(f \mapsto \int_X f\) это линейный функционал на \(\mathcal{L}(X)\) % TODO: #75 определить функционал
...
\begin{proof}
    Суммируемость \(f_n, f\), а также утверждение ``и тем более'' доказываются так же, как в теореме \ref{лебега}.
    ...
    \(2g - h_n \geq 0\) и возрастает как последовательность функций, \(2g - h_n\to 2g\) почти везде. Тогда по теореме \ref{леви}:
    \[\int_X 2g - h_n \to \int_X 2g \Rightarrow \int_X h_n \to 0\]
    \[\int_X |f_n - f| \leq \int_X h_n \to 0\]

    \begin{itemize}
        \item \eqref{тривиально неотрицательно}: по построению
        \item \eqref{по 2g}: по \eqref{|f_n - f| < 2g}
        \item \eqref{по fn to f}: по \eqref{fn to f}
    \end{itemize}
\end{proof}
