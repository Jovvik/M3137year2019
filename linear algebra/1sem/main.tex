...
Возьмём произвольную прямую линию $l$ в $\R^2$ или $\R^3$. Пусть $\vec s=\begin{pmatrix} m & n & p\end{pmatrix}\tran $ --- направляющий вектор этой прямой. Зафиксируем точку $M_0$ на $l$ с радиус-вектором $\vec r_0=\begin{pmatrix}x_0&y_0&z_0\end{pmatrix}\tran $ и выразим точку $M$ на $l$ с радиус-вектором $\vec r=\begin{pmatrix}x&y&z\end{pmatrix}\tran $.
...
Если даны две точки на прямой $M_0$ и $M_1$ с радиус-вектором $\vec r_1=\begin{pmatrix}x_1& y_1& z_1\end{pmatrix}\tran $, можем получить направляющий вектор для $l$, который равен $\vec r_1-\vec r_0$. Подставим это в векторное уравнение прямой:
...
Примеры: \begin{enumerate}
    \item $X=\{x=\begin{pmatrix}
                  \xi^1 & \ldots & \xi^n
              \end{pmatrix}\tran , \xi^i\in\R\}$ \textit{(или $\mathbb{C}$)}
