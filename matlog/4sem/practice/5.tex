    ...
    \item Назовём \emph{древовидной} моделью Крипке модель, в которой множество
          ...
          \begin{enumerate}
              \item Докажите, что любое высказывание, опровергаемое моделью Крипке, может
                    ...
                    % Докажем по индукции по числу операторов в формуле.

                    % \begin{itemize}
                    %     \item [База:] \(n = 0\) не опровергаемо, поэтому пусть \(n = 1\). Тогда несложным перебором можно убедиться, что искомое верно.
                    %     \item [Переход:] 4 случая:
                    %           \begin{enumerate}
                    %               \item \(\alpha = \neg \beta\). Мы хотим, чтобы \(W_i \nVdash \alpha\), т.е. \(\exists W_j : W_i \leq  W_j\) и \(W_j \Vdash \beta\).
                    %           \end{enumerate}
                    % \end{itemize}
