...
\[\E\xi = \int_{ -\infty}^{+\infty} x f_\xi(x) dx = \int_a^b x \frac{1}{b - a}dx = \frac{b^2 - a^2}{2(b - a)} = \frac{a + b}{2} \]
\[\E\xi^2 = \int_{ -\infty}^{+\infty} x^2 f_\xi(x) dx = \int_a^b x^2 \frac{dx}{b - a} = \frac{a^2 + ab + b^2}{3}\]
\[\D\xi = \E\xi^2 - (\E\xi)^2 = \frac{a^2 + ab + b^2}{2} - \left( \frac{a + b}{2} \right)^2 = \frac{a^2 + b^2 - 2ab}{12} = \frac{(b - a)^2}{12}\]
...
\[\E\xi^k = \int_{ -\infty}^{+\infty} x^k f_\xi(x) dx = \int_0^{+\infty} x^k \alpha e^{ - \alpha x} dx = \frac{1}{\alpha^k} \int_0^{+\infty} (\alpha x)^k \alpha e^{ - \alpha k} d(\alpha x) = \frac{k!}{\alpha^k} \]
\[\E\xi = \frac{1}{\alpha}\]
\[\E\xi^2 = \frac{2}{\alpha^2}\]
\[\D\xi = \frac{2}{\alpha^2} - \left( \frac{1}{\alpha} \right)^2 = \frac{1}{\alpha^2}\]
\[\sigma = \frac{1}{\alpha}\]
...
\[\E\xi = a \quad \sigma \xi = \sigma \quad \D\xi = \sigma^2\]
...
\begin{align*}
    \E\xi & = \int_{-\infty}^{+\infty} x \varphi(x) dx                                                                                   \\
...
\begin{align*}
    \D\xi & = \int_{-\infty}^{+\infty} x^2 \varphi(x) dx - (\E\xi)^2                                                                                      \\
...
\begin{enumerate}
    ...
    \item Пусть \(\xi \in N(a, \sigma^2)\). Тогда \(\E\xi = a, \D\xi = \sigma^2\).
          \begin{proof}
              \[\eta : = \frac{\xi - a}{\sigma} \in N(0, 1)\]
              \[\E\xi = \sigma \E\xi + a = 0 + a = a\]
...
\begin{definition}
    \textbf{Асимметрией} распределения называется число \(A_s = \E\left( \frac{\xi - a}{\sigma} \right)^3 = \frac{M_3}{\sigma^3}\)
\end{definition}

\begin{definition}
    \textbf{Эксцессом} распределения называется число \(E_k = \E\left( \frac{\xi - a}{\sigma} \right)^4 - 3 = \frac{M_4}{\sigma^4} - 3\)
\end{definition}
...
%<*27>
\begin{definition}
    \textbf{Гамма-функция Эйлера} \(G(\lambda) = \int_0^{+\infty} t^{\lambda - 1} e^{ - t}dt\). При \(\lambda \in \N\) \(\Gamma(\lambda + 1) = \lambda!\)
\end{definition}
...
\[F_\xi(x) = \frac{\alpha^\lambda}{\Gamma(\lambda)} \int_0^x t^{\lambda - 1} e^{ - \alpha t} dt , \quad x \geq 0\]
Если \(\lambda\in\N\), то:
